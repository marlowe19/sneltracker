...
export default function TimerSectionClient({
  user,
  activeEntries = [],
  pendingTimers = [],
  setPendingTimers,
  ...
  const [stoppedTimers, setStoppedTimers] = useState({}); // entryId -> stopTime
  const [projects, setProjects] = useState([]);
  const [openDropdowns, setOpenDropdowns] = useState({}); // timerId -> boolean
      ...
      Object.keys(openDropdowns).forEach((timerId) => {
        if (openDropdowns[timerId] && dropdownRefs.current[timerId]) {
          if (!dropdownRefs.current[timerId].contains(event.target)) {
            setOpenDropdowns((prev) => ({ ...prev, [timerId]: false }));
          }
        }
      });
    }
  ...
  function toggleDropdown(timerId) {
    setOpenDropdowns((prev) => ({
      ...prev,
      [timerId]: !prev[timerId],
    }));
  }

  function closeDropdown(timerId) {
    setOpenDropdowns((prev) => ({ ...prev, [timerId]: false }));
  }

  function handleRemovePendingTimer(timerId) {
    if (setPendingTimers) {
      setPendingTimers((prev) => prev.filter((t) => t.id !== timerId));
    }
  }
    ...
    if (entryId.startsWith("pending-")) {
      ...
      if (setPendingTimers) {
        setPendingTimers((prev) =>
          prev.map((t) =>
            t.id === entryId ? { ...t, projectId: newProjectId } : t
          )
        );
      }
    } else {
  ...
  async function handleStart(timer) {
    try {
      ...
      // Remove from pending
      if (setPendingTimers) {
        setPendingTimers((prev) => prev.filter((t) => t.id !== timer.id));
      }
      startTransition(() => router.refresh());
    } catch (error) {
  ...
  function handleStopClick(entryId, stopTime) {
    setStoppedTimers((prev) => ({ ...prev, [entryId]: stopTime }));
  }

  async function handleStop(entry) {
    const stopTime = new Date().toISOString();
    handleStopClick(entry.id, stopTime);
    try {
