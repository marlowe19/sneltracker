-- Migration 052: Create get_day_expenses_v2 (optional — app uses expensesService.fetchVisibleExpenses instead)
-- Members with role 'owner' in project_members see all team expenses for that
-- shared project on the given day, matching get_day_entries_v4 visibility.

DROP FUNCTION IF EXISTS get_day_expenses_v2(TEXT, DATE);

CREATE OR REPLACE FUNCTION get_day_expenses_v2(
  p_user_name TEXT,
  p_day_date DATE
)
RETURNS TABLE (
...
BEGIN
  ...
  WHERE e.date >= p_day_date::TIMESTAMP
    AND e.date < (p_day_date + INTERVAL '1 day')::TIMESTAMP
    AND (
...
COMMENT ON FUNCTION get_day_expenses_v2 IS
'Fetches expenses for a specific day (v2). Returns the user''s own expenses plus all
